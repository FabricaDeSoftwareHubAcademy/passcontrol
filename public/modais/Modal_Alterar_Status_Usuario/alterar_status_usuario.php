<div class="fundo_AltStatusUsu modalAltStatus" id="modalAltStatus">
        <section class="modal_AltStatusUsu">
            <img src="../../../public/img/img-modais/Logo Nota Controlnt.png" alt="Logo Nota Control" class="logo_AltStatusUsu">
            <h1 class="titulo_AltStatusUsu">Confirmação</h1>
            <hr class="linha_AltStatusUsu">
            <p class="texto_AltStatusUsu"><b>Deseja Alterar o Status Deste Usuário?</b></p>
            <div class="button-group_AltStatusUsu">
                <button type="button" class="botao-modal_AltStatusUsu cancel-atv-inatv">Não</button>
                <button type="button" class="botao-modal_AltStatusUsu save-atv-inatv">Sim</button>
            </div>
        </section>
    </div>