<?php
    ...
    class FilaSenha {
        ...
        public function inserirFilaSenha($nome, $servico, $categoria) {
            ...
            $ultimaSenha = $this->db->select("prioridade_fila_senha = $categoria", "fila_senha_created_in DESC", 1, "senha_fila_senha");
            $proximaSenha = 1;
                if($ultimaSenha) {
                    $resultado = $ultimaSenha->fetch(PDO::FETCH_ASSOC);
                        if ($resultado) {
                            $proximaSenha = $resultado['senha_fila_senha'] + 1;
                        }
                }

            $senhaGerada = $prefixo . str_pad($proximaSenha, 3, '0', STR_PAD_LEFT);
